...
function renderUserTable(users) {
    ...
    tbody.innerHTML = users.map(user => {
        ...
        const adminLink = !isDev ? (!isAdmin ? `
            <button class="btn btn-sm btn-outline-danger toggle-admin-btn" data-uid="${user.uid}" style="border-color: #F22;">
                <span class="material-symbols-outlined fs-6">encrypted_add_circle</span>
            </button>` : `
            <button class="btn btn-sm btn-danger toggle-admin-btn" data-uid="${user.uid}" style="color: white; border-color: #F22; background-color: #F22;">
                <span class="material-symbols-outlined fs-6">encrypted_minus_circle</span>
            </button>`) : '';
        const deletionLink = !isDev ? `
            <button class="btn btn-sm btn-outline-danger delete-btn" data-uid="${user.uid}" style="border-color: #F22;">
