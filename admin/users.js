...
function renderUserTable(users) {
    ...
    tbody.innerHTML = users.map(user => {
        ...
        const adminLink = !isDev ? (!isAdmin ? `
            <button class="btn btn-sm btn-outline-danger toggle-admin-btn" data-uid="${user.uid}" style="border-color: #F44 !important; color: #F44 !important; display: flex; align-items: center; justify-content: center;">
                <span class="material-symbols-outlined fs-6">encrypted</span>
            </button>` : `
            <button class="btn btn-sm btn-danger toggle-admin-btn" data-uid="${user.uid}" style="color: white; border-color: #F44 !important; background-color: #F44 !important; display: flex; align-items: center; justify-content: center;">
                <span class="material-symbols-outlined fs-6">encrypted_off</span>
            </button>`) : '';
        const deletionLink = !isDev ? `
            <button class="btn btn-sm btn-outline-danger delete-btn" data-uid="${user.uid}" style="border-color: #F44 !important; color: #F44 !important; display: flex; align-items: center; justify-content: center;">
